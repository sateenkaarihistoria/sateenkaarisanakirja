...
const Kayttajat = () => {
  ...
  useEffect(() => {
    setLadataan(true)
    getSuojattuData('/api/kayttaja', sessioData.token).then(result => {
  ...
  function poistaKayttaja(id) {
    deleteData('/api/kayttaja/', id, sessioData.token).then(result => {
      if (result.status === 'success') {
        const muutettuKayttajat = kayttajat.filter(kayttaja => kayttaja.id !== id);
  ...
  return (
    <Container>
      ...
      { errors 
        ? <Message negative><Message.Header>Virhe</Message.Header>{ errors }</Message> 
        : (ladataan 
