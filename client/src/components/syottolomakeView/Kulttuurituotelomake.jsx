    ...
    getSuojattuData('/api/kulttuuriteoslomake', user.token).then((result) => {
      if (result.status === 'success') {
        const mappedResults = result.data.reduce((p, r) => {
          ...
          const objkey = key === 'kuvaus' ? 'asiasana' : key;
  ...
  const tarkistaKulttuurituoteLomake = async () => {
    ...
    if (!virhe) {
      const luoFormiobjekti2 = {
        ...
        henkilo_paikkakunta: muunnaKirjainIsoksi(
          kulttuurituoteTila.paikkakunta,
        ),
        ...
        teos_paikkakunta: muunnaKirjainIsoksi(
          kulttuurituoteTila.tapahtumapaikkakunta,
        ),
