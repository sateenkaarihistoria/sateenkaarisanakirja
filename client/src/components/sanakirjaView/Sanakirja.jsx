...
import { Container, Loader, Segment, Grid, Responsive } from 'semantic-ui-react';
...
const SanakirjaPlain = ({ className }) => {
  ...
  const suodataIlmentymatAsiasanalla = suodataKokoelma ('ilmentymat') ('asiasana')
  ...
  const hakusanojenSuodatus = () => {
    ...
    if (suodatusPaalla) {
      switch (suodatusoptio) {
        case 'kirjainhaku':
          suodatetutAsiasanat = suodataHakusanalla (hakutermiTrim) (predikaatti) (asiasanat)
          break;    
        case 'hakusana':
          suodatetutAsiasanat = suodataHakusanalla (hakutermiTrim) (predikaatti) (asiasanat)
          break;
        case 'asiasana':
          suodatetutAsiasanat = suodataIlmentymatAsiasanalla (hakutermiTrim) (predikaatti) (asiasanat)
          break;
        default:
    ...
    return (
      <>
        {suodatetutAsiasanat.map((item, index) => {
          return (
          <Grid.Row key={index}>
            <div className="menuitem" onClick={()=>setAktiivinenAsiasana(item)}>
              <b>{item.sana}</b>  {'(' + item.sanaluokka + ')' }
            </div>
          </Grid.Row>
          )
        })}
  ...
  const poistoHandler = poistettava => as_id => (ilm_id) => {
    switch (poistettava) {
      case 'hakusana':
        deleteData('/api/hakusana/', as_id, sessioData.token)
        .then(result => {
          if (result.status === 'success') {
            haeAsiasanat().then(() => {
        ...
        break;
      case 'ilmentyma':
        deleteData('/api/ilmentyma/', ilm_id, sessioData.token)
        .then(result => {
          if (result.status === 'success') {
            haeAsiasanat().then(() => {
              setAktiivinenAsiasana(null)
  ...
  const updateHandler = muutettava => uusiData => {
    ...
    switch (tyyppi) {
      case 'hakusana':
        putData('/api/hakusana/', uusiData, id, sessioData.token)
        .then(result => {
          if (result.status === 'success') {
            haeAsiasanat().then(() => {
              setAktiivinenAsiasana(null)
        ...
        break;
      case 'ilmentyma':
        putData('/api/ilmentyma/', uusiData, id, sessioData.token)
        .then(result => {
          if (result.status === 'success') {
            haeAsiasanat().then(() => {
              setAktiivinenAsiasana(null)
