...
exports.listOrganizations = async function (req, res, next) {
  const result = await haeOrganisaatiot();
  res.status(200).json(result);
};

const haeOrganisaatiot = async () => {
  // Etsitään kaikki organisaatiot
  const organisaatiot = await new Promise((resolve, reject) =>
    connection.query("SELECT * FROM organisaatio", (error, results) => {
      if (error) {
        reject(error);
      } else {
  ...
  const res = await haeTiedot(organisaatiot);
  return res;
};

const haeTiedot = async (organisaatiot) => {
  // palautettava objekti
  const palautus = { organisaatiot: [] };

  for (let j = 0; j < organisaatiot.length; j++) {
    const org = organisaatiot[j];
    // Etsitään organisaation tapahtumat
    const tapahtumat = await new Promise((resolve, reject) =>
      connection.query(
        "SELECT * FROM tapahtuma WHERE organisaatio_id = $1",
        [org.id],
        (error, results) => {
          if (error) {
            reject(error);
            ...
            const kysely =
              "SELECT DISTINCT (kuvaus) FROM tapahtuma, asiasana, liittyy WHERE asiasana.id = asiasana_id AND tapahtuma.id = tapahtuma_id AND tapahtuma_id = $1";
    ...
    const kysely =
      "SELECT maa, paikkakunta FROM sijainti, org_toimii, organisaatio WHERE organisaatio_id = $1 AND sijainti.id = sijainti_id AND organisaatio.id = organisaatio_id";
    const sijainti = await new Promise((resolve, reject) =>
      connection.query(kysely, [org.id], (error, results) => {
        if (error) {
          reject(error);
        } else {
          resolve(results.rows);
...
exports.returnOrganization = function (req, res, next) {
  ...
  connection.query(
    "SELECT * FROM organisaatio WHERE id = $1",
    [id],
    (error, results) => {
      if (error) {
        next(error);
        ...
        res
          .status(404)
          .json({ error: "Organization with given id does not exist!" });
      } else {
...
exports.insertOrganization = function (req, res, next) {
  ...
  const kysely =
    "INSERT INTO organisaatio (nimi) VALUES ($1) ON CONFLICT (nimi) DO UPDATE SET nimi=EXCLUDED.nimi RETURNING id";
...
exports.tarkistaTapahtuma = function (req, res, next) {
  ...
  const kysely =
    "SELECT * FROM tapahtuma WHERE nimi = $1 AND paivays = $2 AND luonne = $3 AND organisaatio_id = $4";
  connection.query(
    kysely,
    [tapahtuma_nimi, paivays, luonne, req.orgId],
    (error, results) => {
      if (error) {
        return next(error);
      }
      if (results.rows[0] == null) {
        ...
        return res
          .status(400)
          .json({ error: "Kyseinen tapahtuma on jo olemassa!" });
      }
    }
  );
};
...
exports.insertEvent = function (req, res, next) {
  ...
  connection.query(
    "INSERT INTO tapahtuma (nimi, paivays, luonne, organisaatio_id, valmis, viesti) VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT ON CONSTRAINT uniikki_tapahtuma DO UPDATE SET nimi=EXCLUDED.nimi RETURNING *",
    [tapahtuma_nimi, paivays, luonne, req.orgId, valmis, viesti],
    (error, results) => {
      if (error) {
        return next(error);
      }
...
exports.deleteLocation = function (req, res, next) {
  ...
  if (org_nimi == null || maa == null || paikkakunta == null) {
    return res
      .status(500)
      .json({ error: "org_nimi, maa, paikkakunta need values!" });
  }

  connection.query(
    "DELETE FROM org_toimii WHERE organisaatio_id = $1",
    [id],
    (error, results) => {
      if (error) {
        next(error);
...
exports.addLocation = function (req, res, next) {
  ...
  // Lisataan sijainti
  let sijaintiId = connection.query(
    "INSERT INTO sijainti (maa, paikkakunta) VALUES ($1, $2) ON CONFLICT (maa, paikkakunta) DO UPDATE SET maa=EXCLUDED.maa RETURNING id",
    [maa, paikkakunta],
    (error, results) => {
      if (error) {
        next(error);
        ...
        connection.query(
          "INSERT INTO org_toimii (organisaatio_id, sijainti_id) VALUES ($1, $2) ON CONFLICT DO NOTHING",
          [req.orgId, sijaintiId],
          (error, results) => {
            if (error) {
              next(error);
            } else {
...
exports.updateOrg = function (req, res, next) {
  ...
  connection.query(
    "UPDATE organisaatio SET nimi = $1 WHERE id = $2 RETURNING *",
    [org_nimi, id],
    (error, results) => {
      if (error) {
        res
          .status(500)
          .json({ error: "Organization with given name already exists!" });
      } else if (results.rows[0] == null) {
        res
          .status(404)
          .json({ error: "Organization with given id does not exist!" });
...
exports.deleteAsiasana = function (req, res, next) {
  ...
  connection.query(
    "DELETE FROM liittyy WHERE tapahtuma_id = $1",
    [id],
    (error, results) => {
      if (error) {
        next(error);
      ...
      let asiasanaId = connection.query(
        ...
        (error, results) => {
          if (error) {
            ...
            connection.query(
              "INSERT INTO liittyy (asiasana_id, tapahtuma_id) VALUES ($1, $2) ON CONFLICT DO NOTHING",
              [asiasanaId, req.tapahtuma_id],
              (error, results) => {
                if (error) {
                  next(error);
                }
...
exports.updateEvent = function (req, res, next) {
  ...
  connection.query(
    "UPDATE tapahtuma SET nimi = $1, luonne = $2, paivays = $3, valmis = $4, viesti = $5 WHERE id = $6 RETURNING *",
    [tapahtuma_nimi, luonne, paivays, valmis, viesti, id],
    (error, results) => {
      if (error) {
        next(error);
      } else if (results.rows[0] == null) {
...
exports.deleteOrganization = function (req, res, next) {
  ...
  connection.query(
    "DELETE FROM organisaatio WHERE id = $1 RETURNING *",
    [id],
    (error, results) => {
      if (error) {
        next(error);
...
exports.deleteEvent = function (req, res, next) {
  ...
  connection.query(
    "DELETE FROM tapahtuma WHERE id = $1 RETURNING *",
    [id],
    (error, results) => {
      if (error) {
        next(error);
