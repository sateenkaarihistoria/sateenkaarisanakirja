...
exports.insertHenkilo = function (req, res, next) {
  ...
  const kysely =
    "INSERT INTO henkilo (etunimi, sukunimi, ammattinimike) VALUES ($1, $2, $3) ON CONFLICT (etunimi, sukunimi, ammattinimike) DO UPDATE SET etunimi=EXCLUDED.etunimi RETURNING id";
  connection.query(
    kysely,
    [etunimi, sukunimi, ammattinimike],
    (error, results) => {
      if (error) {
        return next(error);
      }
...
exports.tarkistaTeos = function (req, res, next) {
  ...
  const kysely = "SELECT * FROM teos WHERE nimi = $1 AND lajityyppi = $2";
  connection.query(kysely, [nimi, lajityyppi], (error, results) => {
    ...
    if (results.rows[0] == null) {
      ...
      connection.query(
        "SELECT * FROM tekee WHERE henkilo_Id = $1 AND teos_Id = $2",
        [req.henkiloId, results.rows[0].id],
        (error, results) => {
          if (error) {
            return next(error);
          ...
          if (results.rows[0] == null) {
            ...
            return res
              .status(400)
              .json({
                error: "Kyseinen teos on jo olemassa kyseisella henkilolla!",
              });
          }
        }
      );
...
exports.insertTeos = function (req, res, next) {
  ...
  connection.query(
    "INSERT INTO teos (nimi, lajityyppi, valmis, viesti) VALUES ($1, $2, $3, $4) ON CONFLICT (nimi, lajityyppi) DO UPDATE SET nimi=EXCLUDED.nimi RETURNING *",
    [nimi, lajityyppi, valmis, viesti],
    (error, results) => {
      if (error) {
        return next(error);
      }
      ...
      connection.query(
        "INSERT INTO tekee (henkilo_id, teos_id) VALUES ($1, $2) ON CONFLICT DO NOTHING",
        [req.henkiloId, results.rows[0].id],
        (error, results2) => {
          if (error) {
            return next(error);
...
exports.insertTeosSijainti = function (req, res, next) {
  ...
  if (teos_maa != null) {
    ...
    connection.query(kysely, [teos_maa, teos_paikkakunta], (error, results) => {
      ...
      connection.query(
        "INSERT INTO tapahtuu_teos (sijainti_id, teos_id) VALUES ($1, $2) ON CONFLICT DO NOTHING",
        [results.rows[0].id, req.teos_id],
        (error, results) => {
          if (error) {
            return next(error);
          }
...
exports.deleteAsiasana = function (req, res, next) {
  ...
  connection.query(
    "DELETE FROM sisaltaa WHERE teos_id = $1 RETURNING *",
    [id],
    (error, results) => {
      if (error) {
        next(error);
      ...
      let asiasanaId = connection.query(
        ...
        (error, results) => {
          if (error) {
            ...
            connection.query(
              "INSERT INTO sisaltaa (asiasana_id, teos_id) VALUES ($1, $2) ON CONFLICT DO NOTHING",
              [asiasanaId, req.teos_id],
              (error, results) => {
                if (error) {
                  next(error);
                }
...
exports.deleteTeosSijainti = function (req, res, next) {
  ...
  connection.query(
    "DELETE FROM tapahtuu_teos WHERE teos_id = $1",
    [id],
    (error, results) => {
      if (error) {
        next(error);
...
exports.updateTeos = function (req, res, next) {
  ...
  connection.query(
    "UPDATE teos SET nimi = $1, lajityyppi = $2, valmis = $3, viesti = $4 WHERE id = $5 RETURNING *",
    [nimi, lajityyppi, valmis, viesti, id],
    (error, results) => {
      if (error) {
        next(error);
      } else if (results.rows[0] == null) {
...
exports.deleteLocation = function (req, res, next) {
  ...
  if (
    ...
    ammattinimike == null ||
    henkilo_maa == null ||
    henkilo_paikkakunta == null
  ) {
    return res
      .status(500)
      .json({
        error:
          "etunimi, sukunimi, ammattinimike, henkiloMaa, henkilo_paikkakunta need values!",
      });
  }

  connection.query(
    "DELETE FROM toimii_henkilo WHERE henkilo_id = $1",
    [id],
    (error, results) => {
      if (error) {
        next(error);
...
exports.addLocation = function (req, res, next) {
  ...
  let sijaintiId = connection.query(
    "INSERT INTO sijainti (maa, paikkakunta) VALUES ($1, $2) ON CONFLICT (maa, paikkakunta) DO UPDATE SET maa=EXCLUDED.maa RETURNING id",
    [henkilo_maa, henkilo_paikkakunta],
    (error, results) => {
      if (error) {
        next(error);
      } else {
        ...
        connection.query(
          "INSERT INTO toimii_henkilo (sijainti_id, henkilo_id) VALUES ($1, $2) ON CONFLICT DO NOTHING",
          [sijaintiId, req.henkiloId],
          (error, results) => {
            if (error) {
              next(error);
            } else {
...
exports.updatePerson = function (req, res, next) {
  ...
  // Paivitetaan henkilo
  connection.query(
    "UPDATE henkilo SET etunimi = $1, sukunimi = $2, ammattinimike = $3 WHERE id = $4 RETURNING *",
    [etunimi, sukunimi, ammattinimike, id],
    (error, results) => {
      if (error) {
        res
          .status(500)
          .json({
            error:
              "Person with given profession already exists! Give another name or profession!",
          });
      } else if (results.rows[0] == null) {
  ...
  connection.query(
    ...
    (error, results) => {
      if (error) {
        ...
        // Poistaa teoksen
        connection.query(
          "DELETE FROM teos WHERE id = $1 RETURNING *",
          [req.teos_id],
          (error, results) => {
            if (error) {
              next(error);
              ...
              res
                .status(404)
                .json({ error: "Culture with given id does not exist!" });
            } else {
...
exports.poistaHenkiloYhteys = function (req, res, next) {
  ...
  connection.query(
    "DELETE FROM tekee WHERE teos_id = $1 RETURNING *",
    [id],
    (error, results) => {
      if (error) {
        next(error);
...
exports.deleteTeokset = function (req, res, next) {
  const kysely = "DELETE FROM teos WHERE id NOT IN (SELECT teos_id FROM tekee)";
...
exports.deletePeople = function (req, res, next) {
  const kysely =
    "DELETE FROM henkilo WHERE id NOT IN (SELECT henkilo_id FROM tekee)";
...
exports.deletePerson = function (req, res, next) {
  ...
  connection.query(
    "DELETE FROM henkilo WHERE id = $1 RETURNING *",
    [id],
    (error, results) => {
      if (error) {
        next(error);
