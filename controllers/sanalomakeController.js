...
exports.returnPaivays = function (req, res, next) {
  connection.query(
    "SELECT DISTINCT sivunumero FROM ilmentyma",
    (error, results) => {
      if (error) {
        next(error);
      } else {
        const tiedot = [];
        const paivays = { paivays: [] };
...
exports.returnHS_osio = function (req, res, next) {
  connection.query(
    "SELECT DISTINCT hs_osio FROM ilmentyma",
    (error, results) => {
      if (error) {
        next(error);
      } else {
        const hs_osio = { hs_osio: [] };
        for (const i in results.rows) {
...
exports.returnSelite = function (req, res, next) {
  connection.query(
    "SELECT DISTINCT selite FROM ilmentyma",
    (error, results) => {
      if (error) {
        next(error);
      } else {
...
exports.returnSanaluokka = function (req, res, next) {
  connection.query(
    "SELECT DISTINCT sanaluokka FROM hakusana",
    (error, results) => {
      if (error) {
        next(error);
      } else {
        const sanaluokka = { sanaluokka: [] };
        for (const i in results.rows) {
...
exports.returnKayttoala = function (req, res, next) {
  connection.query(
    "SELECT DISTINCT kayttoala FROM ilmentyma",
    (error, results) => {
      if (error) {
        next(error);
      } else {
        const kayttoala = { kayttoala: [] };
        for (const i in results.rows) {
