...
exports.uusiKayttaja = function (req, res, next) {
  if (req.body.nimi && req.body.salasana && req.body.rooli) {
    ...
    const kysely =
      "INSERT INTO kayttaja (nimi, salasana, rooli) VALUES ($1, $2, $3) RETURNING *";
    ...
    res
      .status(400)
      .json({ status: 400, error: "Pyynnöstä puuttuu parametreja" });
  }
};
...
exports.muutaKayttaja = function (req, res, next) {
  ...
  const kysely =
    "UPDATE kayttaja SET nimi = $1, rooli = $2 WHERE id = $3 RETURNING *";

  connection.query(
    kysely,
    [req.body.nimi, req.body.rooli, req.params.id],
    (error, results) => {
      if (error) {
        return next(error);
      }
...
exports.muutaOmatTiedot = function (req, res, next) {
  if (req.body.nimi && req.body.uusiSalasana) {
    ...
    const kysely =
      "UPDATE kayttaja SET nimi = $1, salasana = $2 WHERE id = $3 RETURNING *";

    bcrypt.hash(uusiSalasana, saltRounds, (err, hash) => {
      const salattuSana = hash;

      connection.query(
        kysely,
        [nimi, salattuSana, req.params.id],
        (error, results) => {
          if (error) {
            return next(error);
          }
          if (!results.rows[0]) {
            ...
            res
              .status(404)
              .json({ status: 404, error: "Käyttäjää ei löydetty" });
          } else {
            console.log("Käyttäjän tiedot päivitetty");
            res.status(200).json(results.rows[0]);
          }
...
exports.poistaKayttaja = function (req, res, next) {
  connection.query(
    "DELETE FROM kayttaja WHERE id = $1",
    [req.params.id],
    (error, results) => {
      if (error) {
        return next(error);
...
exports.luoAdmin = function (req, res, next) {
  if (req.body.salasana) {
    connection.query(
      "SELECT * FROM kayttaja WHERE nimi = $1",
      ["Admin"],
      (error, results) => {
        if (error) {
          next(error);
...
exports.tarkistaData = [
  body("nimi")
    ...
    .escape()
    .isLength({ min: 1 })
    .withMessage("Nimi ei saa olla tyhjä"),
  body("salasana")
    .trim()
    .escape()
    .isLength({ min: 8 })
    .withMessage("Salasanan pitää olla 8 vähintään merkkiä"),
  body("rooli")
    .trim()
    .escape()
    .isIn(["admin", "tutkija"])
    .withMessage("Ei sallittu rooli"),
...
exports.tarkistaOmaMuutos = [
  ...
  body("salasana")
    ...
    .escape()
    .isLength({ min: 8 })
    .withMessage("Vanhan salasanan pitää olla 8 vähintään merkkiä"),
  body("uusiSalasana")
    .trim()
    .escape()
    .isLength({ min: 8 })
    .withMessage("Uuden salasanan pitää olla 8 vähintään merkkiä"),
  function (req, res, next) {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(422).json({ errors: errors.array() });
