...
exports.returnOrgNimi = function (req, res, next) {
  connection.query(
    "SELECT DISTINCT nimi FROM organisaatio",
    (error, results) => {
      if (error) {
        next(error);
      } else {
        const tiedot = [];
        const nimet = { org_nimi: [] };
...
exports.returnTapahtumaNimi = function (req, res, next) {
  connection.query("SELECT DISTINCT nimi FROM tapahtuma", (error, results) => {
    if (error) {
      ...
      const tapahtuma_nimi = { tapahtuma_nimi: [] };
      for (const i in results.rows) {
...
exports.returnTapahtumaLuonne = function (req, res, next) {
  connection.query(
    "SELECT DISTINCT luonne FROM tapahtuma",
    (error, results) => {
      if (error) {
        next(error);
      } else {
        const luonne = { tapahtuma_luonne: [] };
        for (const i in results.rows) {
...
exports.returnVuosi = function (req, res, next) {
  connection.query(
    "SELECT DISTINCT paivays FROM tapahtuma",
    (error, results) => {
      if (error) {
        next(error);
      } else {
